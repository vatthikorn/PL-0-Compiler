...
void printSymbolTable() {
    ...
    //ofp = fopen("/Users/Zack/Development/UCF/Systems Software/Assignment3/Test Cases/symlist.txt", "w");
    ...
    while (source[src_ptr] != '\0' && src_ptr < CODE_LENGTH) {
        ...
        if ((source[src_ptr] >= 'a' && source[src_ptr] <= 'z') || (source[src_ptr] >= 'A' && source[src_ptr] <= 'Z')) {
            ...
            while ((source[src_ptr] >= 'a' && source[src_ptr] <= 'z') ||
                ...
                if (count > MAX_IDENT_LENGTH) {
                    printf("\nError: Variable name is too long: '%s...' at line %d\n\n", identifier, line);
                    exit(1);
                }
        ...
        if (source[src_ptr] >= '0' && source[src_ptr] <= '9') {
            ...
            if ((source[src_ptr] >= 'a' && source[src_ptr] <= 'z') ||
                (source[src_ptr] >= 'A' && source[src_ptr] <= 'Z')) {
                printf("\nError: Bad variable name at line %d\n\n", line);
                exit(3);
            }
            ...
            while (source[src_ptr] >= '0' && source[src_ptr] <= '9') {
                count++;
                if (count > MAX_DIGITS) {
                    printf("\nError: Digit too long at line %d\n\n", line);
                    exit(4);
                }
        ...
        printf("\nError: Invalid symbol '%c' at line %d\n\n", source[src_ptr], line);
        exit(5);
